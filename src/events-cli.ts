...
export function registerEventsCli(api: OpenClawPluginApi, { program }: { program: Command }) {
  program
    ...
    .description("Search for upcoming live events near a location")
    ...
    .action(async (location?: string, opts?: { keyword?: string; days?: string }) => {
      ...
      if (!apiKey) {
        console.error(
          "Error: live-events plugin apiKey not configured.\n" +
            "Set it via: openclaw config set plugins.entries.live-events.config.apiKey YOUR_KEY",
        );
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        console.error(
          `Error fetching events: ${err instanceof Error ? err.message : String(err)}`,
        );
        process.exitCode = 1;
      }
    });
